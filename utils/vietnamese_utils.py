...
def extract_speaker_and_text(combined_text: str) -> Tuple[str, str]:
    """
    Extracts the speaker ID and text from a string like '[User_4]: Hello'.
    If no speaker pattern is found, returns ("", full_text).
    """
    match = re.search(r'\[(.*?)\]:\s*(.*)', combined_text)
    if match:
