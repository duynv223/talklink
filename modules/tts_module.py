...
class TTSModule:
    ...
    def _load_all_models(self):
        # Load unique model_ids once
        unique_model_wrappers = {}
        for model_id in set(self.lang_to_model.values()):
            model_path = self.model_paths[model_id]
            unique_model_wrappers[model_id] = TTSWrapper(model_id, model_path, self.device)

        # Map each language to its model instance
        for lang, model_id in self.lang_to_model.items():
            self.models[lang] = unique_model_wrappers[model_id]
